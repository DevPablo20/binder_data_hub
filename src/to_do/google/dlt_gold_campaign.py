...
spark = get_spark_session(True, "CreateGoldCampaignTables")


def gold_google_campaign_daily_metrics():
    try:
        df = spark.read.format("delta").load(GOLD_DAILY_METRICS)

        grouped_df = df.groupBy(
            col("sub_platform"), col("campaign_id"), col("date")
        ).agg(
        ...
        final_df.write.format("delta").mode("overwrite").save(
            GOLD_CAMPAIGN_DAILY_METRICS
        )
    except Exception as e:
        raise Exception(
            f"Error while processing gold_google_campaign_daily_metrics"
        ) from e


def gold_google_campaign_daily_geo_metrics():
    try:
        df = spark.read.format("delta").load(GOLD_DAILY_GEO_METRICS)

        grouped_df = df.groupBy(
            col("state_id"), col("date"), col(
                "sub_platform"), col("campaign_id")
        ).agg(
            first(col("ad_account_id")).alias("ad_account_id"),
            first(col("ad_account_name")).alias("ad_account_name"),
            first(col("ad_account_status")).alias("ad_account_status"),
        ...
        final_df.write.format("delta").mode("overwrite").save(
            GOLD_CAMPAIGN_DAILY_GEO_METRICS
        )
    except Exception as e:
        raise Exception(
            f"Error while processing gold_google_campaign_daily_geo_metrics"
        ) from e


def gold_google_campaign_daily_age_metrics():
    try:
        df = spark.read.format("delta").load(GOLD_DAILY_AGE_METRICS)

        grouped_df = df.groupBy(
            col("age_id"), col("date"), col("sub_platform"), col("campaign_id")
        ).agg(
            first(col("ad_account_id")).alias("ad_account_id"),
            first(col("ad_account_name")).alias("ad_account_name"),
            first(col("ad_account_status")).alias("ad_account_status"),
        ...
        final_df.write.format("delta").mode("overwrite").save(
            GOLD_CAMPAIGN_DAILY_AGE_METRICS
        )
    except Exception as e:
        raise Exception(
            f"Error while processing gold_google_campaign_daily_age_metrics"
        ) from e


def gold_google_campaign_daily_gender_metrics():
    try:
        df = spark.read.format("delta").load(GOLD_DAILY_GENDER_METRICS)

        grouped_df = df.groupBy(
            col("gender_id"), col("date"), col(
                "sub_platform"), col("campaign_id")
        ).agg(
            first(col("ad_account_id")).alias("ad_account_id"),
            first(col("ad_account_name")).alias("ad_account_name"),
        ...
        final_df.write.format("delta").mode("overwrite").save(
            GOLD_CAMPAIGN_DAILY_GENDER_METRICS
        )
    except Exception as e:
        raise Exception(
            f"Error while processing gold_google_campaign_daily_gender_metrics"
        ) from e


def execute_gold_campaign_dlt():
    try:
        gold_google_campaign_daily_metrics()
        gold_google_campaign_daily_geo_metrics()
        gold_google_campaign_daily_age_metrics()
        gold_google_campaign_daily_gender_metrics()
    except Exception as e:
        raise Exception(f"Error while executing gold_campaign dlt") from e


if __name__ == "__main__":
    execute_gold_campaign_dlt()
